...
const Announcement = () => {
  ...
  const hideAnnouncement = () => {
    ...
    setAnnouncementClass(
      "bg-indigo-600 p-4 transition-opacity duration-500 ease-in-out opacity-0"
    );
  ...
  useEffect(() => {
    if (!localStorage.getItem("hideAnnouncement")) {
      ...
      setAnnouncementClass(
        "bg-blue-200 p-4 transition-opacity duration-500 ease-in-out opacity-1"
      );
    }
  });
        ...
        <div className="max-w-screen-xl mx-auto px-4 py-3 flex items-start justify-between text-white md:px-8">
          <div className="flex gap-x-4">
            ...
            <p className="py-2 font-medium">
              We just added new books! check out{" "}
              <a
                href="/"
